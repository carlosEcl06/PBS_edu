// Exercise 8a: fill in the three blanks (___) so the SEQKIT_STATS step works
//
// A small pipeline for the simulated samples:
//   FASTP -> MINIMAP2 -> SAMTOOLS_SORT -> SAMTOOLS_FLAGSTAT
// plus SEQKIT_STATS on the cleaned reads (exercise 8a)
//
// Run it with:   nextflow run exercise_08a_add_stats.nf -resume
...
process SEQKIT_STATS {
    ...
    script:
    """
    seqkit ___ -T $r1 $r2 > ${id}.stats.tsv
    """
}
...
workflow {
    ...
    // TODO 3/3: call SEQKIT_STATS on the cleaned reads of FASTP, i.e. FASTP.out[0]
    ___(FASTP.out[0])
}
